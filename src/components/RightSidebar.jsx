...
const RightSidebar = ({ theme }) => {
  ...
  return (
    <div className={`rounded-lg h-full w-full flex flex-col overflow-hidden transition-colors duration-200 ${isDark ? 'bg-[#131722]' : 'bg-white'}`}>
      {/* Header with Search and Settings */}
      <div className={`h-[50px] px-3 border-b flex-shrink-0 flex items-center transition-colors duration-200 ${isDark ? 'bg-[#131722] border-[#2a2e39]' : 'bg-white border-gray-200'}`}>
        <div className="flex items-center gap-2 w-full">
          <div className="flex-1 flex items-center gap-2">
            <div className={`text-gray-400 flex items-center justify-center transition-colors ${isDark ? '' : 'text-gray-500'}`}>
            ...
            <input
              ...
              className={`w-full bg-transparent border-none text-[16px] placeholder-gray-500 focus:outline-none h-full transition-colors ${isDark ? 'text-white' : 'text-gray-900'}`}
            />
          </div>
          <button className={`flex items-center justify-center p-1 rounded-full transition-colors ${isDark ? 'hover:bg-[#2a2e39] text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
      ...
      <div className={`flex-1 overflow-auto transition-colors duration-200 ${isDark ? 'bg-[#131722]' : 'bg-white'}`}>
        <table className="w-[520px] min-w-[520px] border-collapse text-left">
          <thead className={`sticky top-0 z-40 border-b transition-colors duration-200 ${isDark ? 'bg-[#131722] border-[#2a2e39]' : 'bg-gray-50 border-gray-200'}`}>
            <tr className="h-[40px]">
              <th className={`sticky left-0 z-50 pl-4 font-semibold text-[14px] w-[160px] transition-colors duration-200 ${isDark ? 'bg-[#131722] text-white' : 'bg-gray-50 text-gray-700'}`}>Symbol</th>
              <th className={`px-3 font-semibold text-[14px] text-center w-[120px] ${isDark ? 'text-white' : 'text-gray-700'}`}>Bid</th>
              <th className={`px-3 font-semibold text-[14px] text-center w-[120px] ${isDark ? 'text-white' : 'text-gray-700'}`}>Ask</th>
              <th className={`px-3 font-semibold text-[14px] text-center w-[120px] ${isDark ? 'text-white' : 'text-gray-700'}`}>Spread</th>
            </tr>
          </thead>
          <tbody>
            {symbolsData.map((item, index) => (
              <tr key={index} className={`h-[40px] border-b transition-colors group cursor-pointer ${isDark ? 'border-[#2a2e39]/50 hover:bg-[#2a2e39]/30' : 'border-gray-100 hover:bg-gray-50'}`}>
                <td className={`sticky left-0 z-30 transition-colors py-2 pl-4 ${isDark ? 'bg-[#131722] group-hover:bg-[#1c202b]' : 'bg-white group-hover:bg-gray-50'}`}>
                  <div className="flex items-center gap-2">
                    {getTrendIcon(item.trend)}
                    <span className={`text-[14px] font-medium whitespace-nowrap ${isDark ? 'text-white' : 'text-gray-900'}`}>{item.symbol}</span>
                  </div>
                </td>
                <td className={`py-2 px-3 text-[14px] text-center ${item.bidClass || (isDark ? 'text-white' : 'text-gray-900')}`}>
                  {item.bid}
                </td>
                <td className={`py-2 px-3 text-[14px] text-center ${item.askClass || (isDark ? 'text-white' : 'text-gray-900')}`}>
                  {item.ask}
                </td>
                <td className={`py-2 px-3 text-[14px] text-center ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {item.spread}
                </td>
              </tr>
            ))}
